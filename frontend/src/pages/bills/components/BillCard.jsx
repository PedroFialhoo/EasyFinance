export default function BillCard({ bill }) {
  ...
  const currentInstallment =
    installments.find(i => !i.paymenDate) || installments[installments.length - 1]

  const installmentNumber = currentInstallment.installmentNumber
  const totalInstallments = bill.numberInstallments

  const isPaid = !!currentInstallment.paymenDate

  const month = new Date(currentInstallment.dueDate).toLocaleDateString("pt-BR", {
    month: "long",
    year: "numeric",
  })

  return (
    <div className="bg-slate-100 rounded-lg p-4 mb-3 shadow flex justify-between items-center w-[30%]  hover:-translate-y-2 transition-discrete">
      <div>
